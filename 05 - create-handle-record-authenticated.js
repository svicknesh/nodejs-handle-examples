...
.then ( authenticatedData => {
    ...
    // create a new Handle Record with the given information
    msgStr = util.format("Creating Handle ID %s", definition.handleid)
    console.log(msgStr)
    ...
    // the values to be sent are a JSON array
    requestBody = []

    // add an administrator type value pair for THIS Handle Record
    hdlAdmin = {}
    hdlAdmin.handle = "25.001/ADMIN/VICKNESH"
    hdlAdmin.index = 302
    hdlAdmin.permissions = "011111110010"

    hdlValue = {}
    hdlValue.index = 101
    hdlValue.type = "HS_ADMIN"
    hdlValue.data = {}
    hdlValue.data.format = "admin"
    hdlValue.data.value = hdlAdmin
    hdlValue.ttl = 86400

    // add this value into the JSON array to be sent across
    requestBody.push(hdlValue)

    // add a normal type value pair
    hdlValue = {}
    hdlValue.index = 1001
    hdlValue.type = "NodeJS Type"
    hdlValue.data = {}
    hdlValue.data.format = "string"
    hdlValue.data.value = "Created from NodeJS"
    hdlValue.ttl = 86400

    // add this value into the JSON array to be sent across
    requestBody.push(hdlValue)

    return axios.put(lhsURL, JSON.stringify(requestBody), requestHeader)
})
.then (result => {
    let hdlResult = result.data

    msgStr = util.format("Create request for Handle ID %s is as follows", definition.handleid)
    console.log(msgStr)

    /*
        If Handle creation is a success, it returns `responseCode` 1 with the Handle ID that was created.
    */
