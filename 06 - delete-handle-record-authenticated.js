...
.then ( authenticatedData => {
    ...
    msgStr = util.format("Deleting Handle ID %s", definition.handleid)
    console.log(msgStr)
    ...
    return axios.delete(lhsURL, requestHeader)
})
.then (result => {
    let hdlResult = result.data

    msgStr = util.format("Delete request for Handle ID %s is as follows", definition.handleid)
    console.log(msgStr)

    /*
        If Handle deletion is a success, it returns `responseCode` 1 with the Handle ID that was created.
    */
